...
from db import save_rembg_image
from remover import ImageRemover, VALID_MODELS
...
# Initialize remover (singleton, lazy-loads sessions on first use)
_remover = ImageRemover()
...
def handler(job: dict) -> dict:
    """RunPod serverless handler for image background removal."""
    ...
    if model not in VALID_MODELS:
        logger.warning("Job %s rejected: unsupported model=%s", runpod_job_id, model)
        return _build_final_response(
            {
                "job_id": runpod_job_id,
    ...
    try:
        ...
        output_image = _remover.remove_background(image, model)
        logger.info(
            "Job %s background removed | output=%sx%s | elapsed=%.2fs",
            runpod_job_id,
            output_image.size[0],
            output_image.size[1],
            time.perf_counter() - rembg_start,
        )
    except Exception as exc:
        logger.exception("Job %s background removal failed", runpod_job_id)
        return _build_final_response(
            {
                "job_id": runpod_job_id,
    ...
    return _build_final_response(response_payload, status="COMPLETED", webhook_enabled=webhook_enabled)
